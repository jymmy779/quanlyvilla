...
import { Lock, Check, Loader2, ArrowLeft, AlertCircle, Home, Sparkles } from 'lucide-react';
...
export default function ResetPasswordPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 md:p-6 relative overflow-hidden font-sans">
      ...
      <div className="max-w-md w-full bg-white/90 backdrop-blur-md rounded-[2.5rem] border border-slate-200 shadow-2xl p-6 md:p-10 space-y-6 md:space-y-8 relative z-10 animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="flex items-center gap-3 justify-center">
          ...
          <span className="text-2xl font-bold text-slate-900">Rentify</span>
        </div>
        ...
        {checkingSession ? (
          <div className="py-12 flex flex-col justify-center items-center gap-3">
            ...
            <p className="text-slate-400 text-sm font-semibold">Đang xác thực liên kết...</p>
          </div>
        ) : errorMsg && !password ? (
          /* Trường hợp lỗi link hết hạn / không hợp lệ */
          <div className="space-y-6 text-center py-4">
            <div className="mx-auto w-16 h-16 bg-red-50 text-red-500 rounded-2xl flex items-center justify-center border border-red-100 animate-bounce">
              <AlertCircle size={32} />
            </div>
            <div className="space-y-2">
              <h2 className="text-lg font-bold text-slate-950">Đường dẫn không hợp lệ</h2>
              <p className="text-slate-500 text-sm leading-relaxed font-semibold">{errorMsg}</p>
            </div>
            <button
          ...
          <div className="space-y-6 text-center py-4 animate-in zoom-in-95 duration-300">
            <div className="mx-auto w-16 h-16 bg-emerald-50 text-emerald-500 rounded-2xl flex items-center justify-center border border-emerald-100 shadow-md">
              <Check size={32} />
            </div>
            <div className="space-y-2">
              <h2 className="text-lg font-bold text-slate-950">Đặt lại mật khẩu thành công!</h2>
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-300">
            <div className="text-center space-y-1.5">
              <h1 className="text-xl md:text-2xl font-bold text-slate-950">Đặt lại mật khẩu</h1>
              <p className="text-slate-400 text-xs md:text-sm font-semibold">Nhập mật khẩu mới cho tài khoản của bạn</p>
            </div>

            {errorMsg && (
              <div className="p-4 bg-red-50 border border-red-100 rounded-2xl flex items-center gap-2.5 text-xs font-semibold text-red-600 leading-relaxed animate-in fade-in duration-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-4" noValidate>
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-1">Mật khẩu mới</label>
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 focus:border-orange-500 focus:ring-4 focus:ring-orange-500/5 rounded-2xl py-3 pl-12 pr-4 outline-none text-xs md:text-sm font-semibold text-slate-800 transition-all"
                    disabled={loading}
                    autoFocus
                  />
                </div>
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-1">Xác nhận mật khẩu</label>
                <div className="relative group">
                  <Lock className="absolute left-4 top-3.5 text-slate-400 group-focus-within:text-orange-500 transition-colors" size={18} />
                  <input
                    type="password"
