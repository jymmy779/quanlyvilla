...
def main():
    ...
    replacements = {
        ...
        "Nền tảng kỹ thuật": "- Nền tảng kỹ thuật: Ứng dụng công nghệ Next.js 16 (React 19, TypeScript), Tailwind CSS v4, Knex.js đa cơ sở dữ liệu (PostgreSQL, MySQL, MSSQL), thư viện Zustand quản lý trạng thái.",
        ...
        "Lý thuyết 2": "Lý thuyết 2 - Knex Query Builder đa cơ sở dữ liệu: Là tầng truy vấn SQL trung gian hỗ trợ đồng thời PostgreSQL, MySQL và MSSQL Server. Cho phép chuyển đổi cơ sở dữ liệu bằng cách thay đổi biến môi trường DB_CLIENT, không cần sửa code ứng dụng.",
        ...
        "Lý thuyết 4": "Lý thuyết 4 - Giao diện Dark Mode Premium: Cơ chế gạt theme tối tự động lưu localStorage và đồng bộ hệ thống giúp người dùng làm ca đêm không bị mỏi mắt.",
        ...
        "Mô tả luồng dữ liệu, API chính, các hàm Supabase, quyền truy cập (RLS).": "Luồng xử lý dữ liệu và kiến trúc API của Rentify:\n1. Tầng truy vấn Knex.js đa nền tảng: Tất cả API route (login, register, CRUD villas/bookings/profiles) đều sử dụng Knex Query Builder thay vì SQL thuần. Hỗ trợ chuyển đổi qua lại giữa PostgreSQL, MySQL, MSSQL Server chỉ bằng biến môi trường DB_CLIENT.\n2. Thuật toán tính giá động đêm gối đầu: Duyệt vòng lặp theo ngày đêm của stay, bóc tách giá trị ngày để so khớp biểu phí ngày thường, Thứ 6, Thứ 7, Chủ Nhật của tháng. Kết quả tổng tiền cộng thêm chi phí dịch vụ phát sinh sẽ hiển thị cho nhân viên xác nhận cọc 50%, hỗ trợ nút khóa giá tay Zalo.\n3. Đồng bộ theme tối Dark Mode: Lắng nghe thay đổi theme hệ thống, gạt nút gạt lưu localStorage, áp dụng class .dark lên thẻ HTML để gạt CSS Tailwind v4 tức thì.\n4. Định dạng số tiền tệ: Xử lý thông minh bắt sự kiện onChange của input số tiền cọc/tổng cộng, định dạng dấu chấm phân cách hàng nghìn động, giữ nguyên vị trí con trỏ chuột thông minh.\n5. Bảo mật đa doanh nghiệp: Dữ liệu được cô lập bằng cột tenant_id trên tất cả các bảng. Mọi truy vấn qua Knex đều tự động gắn điều kiện WHERE tenant_id = ?, kết hợp JWT authentication để xác thực danh tính người dùng.",
        ...
        "Sơ đồ Diagram": "Sơ đồ hoạt động Multi-tenant: Hệ thống cô lập dữ liệu bằng tenant_id trên tất cả các bảng. Mỗi truy vấn đều mang điều kiện WHERE tenant_id = ? thông qua tầng Knex Query Builder, đảm bảo nhân viên Startup A không thể truy cập dữ liệu của Startup B.",
        ...
        "Xử lý 1": "Hệ thống xác thực & phân quyền (JWT + Knex Multi-Dialect): Xử lý đăng nhập, đăng ký của người dùng bằng JSON Web Token. Sử dụng Knex Query Builder để truy vấn cơ sở dữ liệu độc lập với nền tảng (PostgreSQL/MySQL/MSSQL). Tự động gán tenant_id cho các bản ghi mới và lọc dữ liệu đa doanh nghiệp ở tầng truy vấn.",
    ...
    for p in doc.paragraphs:
        ...
        if p_text_stripped in ["Nội dung 1", "Nội dung 2", "Nội dung 3"]:
            ...
            if "Giới thiệu quy trình 1" in h2_context or "Booking & Deposit" in h2_context or "đặt phòng và cọc phòng" in h2_context:
                ...
                if p_text_stripped == "Nội dung 1":
                    replace_text = "Bảo mật đa doanh nghiệp (Multi-tenant Security): Dữ liệu được cô lập bằng cột tenant_id trên tất cả các bảng. Mọi truy vấn qua Knex đều tự động gắn điều kiện WHERE tenant_id = ?. Nhân viên Startup A tuyệt đối không thể truy cập dữ liệu của Startup B."
                elif p_text_stripped == "Nội dung 2":
                    replace_text = "Đồng bộ dữ liệu thời gian thực: Cập nhật ngay lập tức các thay đổi về trạng thái phòng trên lịch điều phối gối đầu khi có nhân viên khác thao tác."
                elif p_text_stripped == "Nội dung 3":
    ...
    rentify_features = [
        ("1", "Xác thực tài khoản (JWT Auth)", "100%", "Đăng nhập, đăng ký, quên mật khẩu, reset mật khẩu qua email link"),
        ("2", "Phân quyền Multi-tenant", "100%", "Cô lập dữ liệu triệt để giữa các doanh nghiệp độc lập qua tenant_id"),
        ...
        ("12", "Knex Query Builder đa nền tảng", "100%", "Hỗ trợ PostgreSQL, MySQL, MSSQL Server chỉ bằng biến môi trường DB_CLIENT")
    ]
